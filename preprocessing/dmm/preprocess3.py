...
from preprocessing.dmm.common3 import *
    ...
    for sample_id in sample_vtype_counts:
        ...
        for vt in Variant.ALL_TYPES_SNV:
            if vt in sample_vtype_counts[sample_id]:
                ...
                cc.append(0)
        c.append(cc)
    median_vtype_counts = dict(list(zip(Variant.ALL_TYPES_SNV, np.median(np.array(c), axis=0))))
...
#    of = tempfile.NamedTemporaryFile(mode='w', delete=False)
    try:
        ...
        with open(hdrf.name, 'w') as f:
            f.write('{}\n'.format(maf.get_file_header())) 
            f.flush()

        with tempfile.NamedTemporaryFile(dir=odir1, delete=False, mode='w') as of:
...
def preprocess(args):
    ...
    if args.generate_negatives > 0 and args.negative_generation_mode in [MODE_NEG_GENERATE, MODE_NEG_AUGMENT]:
        ...
        fns,odir2 = generate_negatives(fns, variant_to_neg_sample, sample_ix_to_id, args) # augmented input files replace the original ones as input
