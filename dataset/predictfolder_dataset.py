...
class PredictFolder(Dataset):
    ...
    def __init__(self, dataset_name = None, 
        ...
        group_class = os.listdir(self.data_dir)

        pd_data = pd.DataFrame()

        for i in group_class:
            all_samples = os.listdir(self.data_dir + i)

            all_samples = [i for i in all_samples if i[0:4]=='new_']

            for j in all_samples:
                get_count = pd.read_csv(self.data_dir + i + '/' + 'count_' + j,index_col=0)['0'].to_list()

                onerow = [i,j] + get_count + [1]

                pd_onerow = pd.DataFrame(onerow).T
                pd_data = pd_data.append(pd_onerow)
    ...
    def load_classinfo(self):
        ...
        if self.dataset_name == 'finalpcawg':
            pd_data = pd.read_csv('./notebookpcawg/classinfo_pcawg.csv',index_col = 0)
