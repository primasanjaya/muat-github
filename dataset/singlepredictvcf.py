...
class SinglePredictVCF(Dataset):
    ...
    def __init__(self, dataset_name = None, 
        ...
        #nm_class,samples,NiSi,SNV,indel,SVMEI,Normal,fold

        samples_names = input_filename[:-4] + '.csv'
        ...
        if self.adddatadir is not None:
            ...
            for i in folder:
                ...
                for j in samples:
                    if j[0:3] == 'new':
                        counter = pd.read_csv(self.adddatadir + i + '/count_' + j[4:],index_col=0)
        ...
        self.vocab_mutation = pd.read_csv('./extfile/dictMutation.csv',index_col=0)
        self.allSNV_index = 0
        ...
        self.pd_position_vocab = pd.read_csv('./extfile/dictChpos.csv',index_col=0)
        self.pd_ges_vocab = pd.read_csv('./extfile/dictGES.csv',index_col=0)
    ...
    def load_classinfo(self):
        ...
        if self.dataset_name == 'finalpcawg':
            pd_data = pd.read_csv('./dataset_utils/classinfo_pcawg.csv',index_col = 0)
            pd_data = pd_data[['class_name','target','n_samples']]
            pd_data.columns = ['class_name','class_index','n_samples']
        ...
        if self.crossdata:
            ...
            for idx,nm_class in enumerate(name_class):
                ...
                samples = [x for x in samples if x[:10]=='count_']
    ...
    def get_data(self,idx):
        ...
        def allgrab(grabcol):
            ...
            if self.NiSi:
                #pdb.set_trace()
                pd_nisi = pd.read_csv(self.data_dir + target_name + '/' + 'NiSi_' + samples,index_col=0)
                pd_nisi = pd_nisi.sample(n = avail_count[0], replace = False)
                pd_nisi = grab(pd_nisi,grabcol)


                if self.SNV:
                    pd_SNV = pd.read_csv(self.data_dir + target_name + '/' + 'SNV_' + samples,index_col=0)
                    ...
                    if self.indel:
                        ...
                        if self.SVMEI:
                            ...
                            if self.Normal:
                                pd_normal = pd.read_csv(self.data_dir + target_name + '/' + 'Normal_' + samples,index_col=0)
        ...
        '''
        datanumeric[0] = torch.zeros(datanumeric[0].shape,dtype = torch.long)
        datanumeric[1] = torch.zeros(datanumeric[1].shape,dtype = torch.long)
        zeros_1 = torch.zeros(datanumeric[0].shape,dtype = torch.long)
        zeros_1[0:10] = 5
        zeros_2 = torch.zeros(datanumeric[1].shape,dtype = torch.long)
        zeros_2[0:10] = 5
        datanumeric[0] = zeros_1
        datanumeric[1] = zeros_2
        '''
